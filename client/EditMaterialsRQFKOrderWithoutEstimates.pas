unit EditMaterialsRQFKOrderWithoutEstimates;
...
type
  TfrmMaterialsRQFKOrderWithoutEstimatesEdit = class(TDialogForm)
    ...
    edtBudgetName: TEdit;
    Label4: TLabel;
    spbBudgetSelect: TSpeedButton;
    spbBudgetClear: TSpeedButton;
    ...
    procedure spbBudgetSelectClick(Sender: TObject);
    procedure spbBudgetClearClick(Sender: TObject);
...
uses ShowENEPRen, ChildFormUnit, ShowENElement, ENElementController,
  ...
  EditMaterialsRQFKOrderOut, RQFKOrderKindController;
...
procedure TfrmMaterialsRQFKOrderWithoutEstimatesEdit.updateFINMaterialsGrid();
var
...
begin
   ...
   sgFINMaterials.RowCount := 2;
      ...
      // NET-4400 Для ордеров вывода из аварийного запаса - брать только с назначения Аварийный запас
      if Self.rqFKOrderObj.kind.code = RQFKORDER_KIND_AVAR_OUT then begin
          AddCondition( condition , ' DAT.REST_PURPOSE_TYPE_ID = ' + IntToStr(REST_PURPOSE_TYPE_ID_AVAR));
      end;

      // NET-4400 Для ордеров ввода в аварийный запас - брать только с назначения Оперативный и Net.Оперативный
      if Self.rqFKOrderObj.kind.code = RQFKORDER_KIND_AVAR_IN then begin
          AddCondition( condition , ' DAT.REST_PURPOSE_TYPE_ID IN (' + IntToStr(REST_PURPOSE_TYPE_ID_OPERATIVE)
   ...
   if rqFKOrderObj.kind.code = RQFKORDER_KIND_AVAR_OUT then begin
        FINMaterialsList := TempFINMaterials.getMaterialsListWithAvar(
                  finFilter,
                  balansNumberCondition,
                  molCode,
                  materialCondition,
...
procedure TfrmMaterialsRQFKOrderWithoutEstimatesEdit.FormShow(Sender: TObject);
var
...
begin
  DisableControls([edtFINMol, edtStorageZoneName, edtBudgetName]);
  ...
  Self.Caption := rqFKOrderObj.kind.name;

end;

procedure TfrmMaterialsRQFKOrderWithoutEstimatesEdit.FormKeyDown(Sender: TObject;
  var Key: Word; Shift: TShiftState);
begin
  if Key = VK_ESCAPE then Exit;
end;

procedure TfrmMaterialsRQFKOrderWithoutEstimatesEdit.btnSaveClick(Sender: TObject);
var
  TempRQFKOrderItem: RQFKOrderItemControllerSoapPort;
  newZoneCode : Integer;
...
begin
  ...
  Case rqFKOrderObj.kind.code  of
    ...
    begin
         TempRQFKOrderItem.addAvarOut(finList, rqFKOrderCode);
    end;
    RQFKORDER_KIND_AVAR_IN:
    begin
        TempRQFKOrderItem.addAvarIn(finList, rqFKOrderCode);
    end;
  End;

  updateFINMaterialsGrid;

  Application.MessageBox(PChar('Строки добавлені!'),
                    PChar('Внимание !'),MB_OK);

end;

procedure TfrmMaterialsRQFKOrderWithoutEstimatesEdit.FormCreate(Sender: TObject);
begin
  inherited;
  MOLCode := '';
  masterMOLCode := '';
...
procedure TfrmMaterialsRQFKOrderWithoutEstimatesEdit.btnFindClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmMaterialsRQFKOrderWithoutEstimatesEdit.sgFINMaterialsCellValidate(
...
procedure TfrmMaterialsRQFKOrderWithoutEstimatesEdit.spbBudgetClearClick(
  Sender: TObject);
begin
    edtBudgetName.Text := '';
    edtCFO.Text := '';
    DisableControls([edtCFO], False);
end;

procedure TfrmMaterialsRQFKOrderWithoutEstimatesEdit.spbBudgetSelectClick(
  Sender: TObject);
  var
   ...
   //     DisableActions([ actNoFilter, actEdit, actInsert ]);
        ...
        begin
            try
              ...
              edtBudgetName.Text := budgetName;

              cfoCode := DMReports.getCFOByBudgetCode(budgetCode);
              if StrToInt(cfoCode) > LOW_INT then begin
                 edtCFO.Text := cfoCode;
                 DisableControls([edtCFO]);
              end;
            except
...
procedure TfrmMaterialsRQFKOrderWithoutEstimatesEdit.spbStorageZoneNameClick(
  Sender: TObject);
...
procedure TfrmMaterialsRQFKOrderWithoutEstimatesEdit.SpeedButton2Click(Sender: TObject);
begin
  inherited;
  zoneCode := LOW_INT;
