...
type
  TfrmENCargoPlanEdit = class(TDialogForm)
  ...
  procedure UpdateGrid(Sender: TObject);
    ...
    procedure chbIsFriableClick(Sender: TObject);
...
var
  frmENCargoPlanEdit: TfrmENCargoPlanEdit;
  //ENPlanWorkObj: ENPlanWork;
...
procedure TfrmENCargoPlanEdit.FormShow(Sender: TObject);
var
...
begin
  ...
  begin
    ...
    SetFloatStyle([edtCountGen, edtNetto, edtLength, edtHeight]);
    DenyBlankValues([edtCountGen, edtNetto, edtLength, edtHeight]);

    HideControls([lblENActNumber, edtENActNumber, spbENAct, lblPK, edtCode,
                 lblExecuter, edtFINExecutorName, spbFINExecutor]);

    // при вставке можно поменять ТИП плана (год, тек, нпз)
    DisableControls([cbPlanWorkKind , spbType ], false);
    DisableControls([spbENPlanWorkState]);

    DisableControls([edtTypeName, spbType, edtWorkState, spbENPlanWorkState, spbENElement, edtENElementName,
                     edtENBudgetName, edtResponsibility, spbENBudget, spbResponsibility,
                     cbENPlanWorkFormName, cbPlanWorkKind, edtKartiName, spbEPKard]);

    gbCargoDescription.Top := 223;
    gbPlanWorkItem.Top := 316;

    // подставить элемент
    ENPlanWorkObj.elementRef := ENElementRef.Create();
    ENPlanWorkObj.elementRef.code := CARGO_OBJECT;
    edtENElementName.Text := 'Вантаж для перевезення';

    // месячный, внеплановый - с работой "перевезення"
    cbPlanWorkKind.ItemIndex := 1;
    gbPlanWorkItem.Visible := true;
    DisableControls([edtKartiNum]);
    edtKartiNum.Text := '0102';
    edtKartiName.Text := 'Перевезення вантажу';

    ENPlanWorkItemObj := ENPlanWorkItem.Create;
    ENPlanWorkItemObj.kartaRef := TKTechCardRef.Create();
    ENPlanWorkItemObj.kartaRef.code := 500004873;
...
procedure TfrmENCargoPlanEdit.FormClose(Sender: TObject;  var Action: TCloseAction);
...
begin
  ...
  begin
    ...
    if (DialogState = dsInsert) and (not NoBlankValues([edtCountGen, edtNetto])) then
    begin
      Application.MessageBox(PChar('Пустые поля недопустимы !'),PChar('Внимание !'),MB_ICONWARNING+MB_OK);
      Action := caNone;
      Exit;
    ...
    if (DialogState = dsInsert) and
      ((not chbIsFriable.Checked) and (not NoBlankValues([edtLength, edtHeight]))) then
    begin
      Application.MessageBox(PChar('Пустые поля недопустимы !'),PChar('Внимание !'),MB_ICONWARNING+MB_OK);
      Action := caNone;
      Exit;
    ...
    if StrToFloat(edtCountGen.Text) < 0.000001 then
    begin
      Application.MessageBox(PChar('Додавати роботу з нульовою відстанню неможна!'), PChar('Увага!'), MB_ICONWARNING);
      ModalResult := mrNone;
      Exit;
    end;
   ...
   ENPlanWorkObj.formRef.code := cbENPlanWorkFormName.ItemIndex + 1;
    ...
    begin
      ...
      case element.typeRef.code of
        ...
        12 : ENPlanWorkObj.code := TempENPlanWork.addByTransport(ENPlanWorkObj);
        ...
        ENELEMENT_TYPE_CARGO_OBJECT : ENPlanWorkObj.code := TempENPlanWork.addByTransport(ENPlanWorkObj);
        else
        begin
      ...
      if frmENCargoPlanEdit.molCode <> '' then
      begin
        TempENMOL2PlanWork :=  HTTPRIOENMOL2PlanWork as ENMOL2PlanWorkControllerSoapPort;
        ...
        ENMOL2PlanWorkObj.molCode := frmENCargoPlanEdit.molCode;
        ENMOL2PlanWorkObj.molName := frmENCargoPlanEdit.molName;
      ...
      if (chbIsFriable.Checked) then
        ENPlanWorkItemObj.commentGen := 'маса, кг: ' + edtNetto.Text + ', насипом.'
      else
        ENPlanWorkItemObj.commentGen := 'маса, кг: ' + edtNetto.Text +
          ', довжина, м: ' + edtLength.Text + ', висота, м: ' + edtHeight.Text + '.';
...
procedure TfrmENCargoPlanEdit.spbENPlanWorkStatusClick(Sender : TObject);
var
...
procedure TfrmENCargoPlanEdit.spbENElementClick(Sender: TObject);
var
...
procedure TfrmENCargoPlanEdit.updateFINMaterialsGrid(estimateItemCode : Integer; stringGrid : TAdvStringGrid );
var
...
procedure TfrmENCargoPlanEdit.pcPlanWorkChange(Sender: TObject);
var
  TempENPlanWorkItem: ENPlanWorkItemControllerSoapPort;
...
procedure TfrmENCargoPlanEdit.actInsertExecute(Sender: TObject);
...
procedure TfrmENCargoPlanEdit.UpdateGrid(Sender: TObject);
Var i, j: Integer;
begin
 if pcPlanWork.ActivePage = tsPlanWorkItems then
 begin
...
procedure TfrmENCargoPlanEdit.actEditExecute(Sender: TObject);
Var TempENPlanWorkItem: ENPlanWorkItemControllerSoapPort;
    TempENEstimateItem: ENEstimateItemControllerSoapPort;
begin
  //if ENPlanWorkObj.status.code <> ENPLANWORKSTATUS_GOOD then
...
procedure TfrmENCargoPlanEdit.actViewExecute(Sender: TObject);
Var TempENPlanWorkItem: ENPlanWorkItemControllerSoapPort;
    TempENEstimateItem: ENEstimateItemControllerSoapPort;
    TempENTransportItem: ENTransportItemControllerSoapPort;
    TempENHumenItem: ENHumenItemControllerSoapPort;
...
procedure TfrmENCargoPlanEdit.actUpdateExecute(Sender: TObject);
begin
  inherited;
  //selectedRow := sgENPlanWorkItem.Row;
  UpdateGrid(Sender);
...
procedure TfrmENCargoPlanEdit.btnPlanReportClick(Sender: TObject);
var
...
procedure TfrmENCargoPlanEdit.actDeleteExecute(Sender: TObject);
Var TempENPlanWorkItem: ENPlanWorkItemControllerSoapPort;
  ObjCode: Integer;
  TempENEstimateItem: ENEstimateItemControllerSoapPort;
  eType : integer;
...
procedure TfrmENCargoPlanEdit.sgENPlanCorrectHistoryDblClick(
  Sender: TObject);
  var
  frmViewOldPlan : TfrmENCargoPlanEdit;
  TempENPlanCorrectHistory : ENPlanCorrectHistoryControllerSoapPort;
  TempENPlanWork: ENPlanWorkControllerSoapPort;
...
begin
   ...
   frmViewOldPlan := TfrmENCargoPlanEdit.Create(Application, dsView);
...
procedure TfrmENCargoPlanEdit.spbENBudgetClick(Sender: TObject);
var
...
procedure TfrmENCargoPlanEdit.spbResponsibilityClick(Sender: TObject);
var
   frmENDepartmentShow: TfrmENDepartmentShow;
   f : ENDepartmentFilter;
begin
...
procedure TfrmENCargoPlanEdit.spbDepartmentClick(Sender: TObject);
var
   frmENDepartmentShow: TfrmENDepartmentShow;
   f : ENDepartmentFilter;
begin
...
procedure TfrmENCargoPlanEdit.spbTypeClick(Sender: TObject);
var
...
procedure TfrmENCargoPlanEdit.edtDateStartClick(Sender: TObject);
begin
//if edtDateStart.Checked then
//begin
...
procedure TfrmENCargoPlanEdit.actMaterialBindingExecute(Sender: TObject);
begin
...
procedure TfrmENCargoPlanEdit.PopupMenu1Popup(Sender: TObject);
begin
...
procedure TfrmENCargoPlanEdit.edtDateStartChange(Sender: TObject);
begin
...
procedure TfrmENCargoPlanEdit.spbENPlanWorkStateClick(Sender: TObject);
var
...
procedure TfrmENCargoPlanEdit.spbENActClick(Sender: TObject);
var
  TempENAct: ENActControllerSoapPort;
  i: Integer;
...
procedure TfrmENCargoPlanEdit.sgENEstimateItemClick(Sender: TObject);
var
  j : Integer;
begin
     // выведем список ФИН материалов .... если они есть ВААЩЕ ...
...
procedure TfrmENCargoPlanEdit.actMaterialBindingToFINExecute(
  Sender: TObject);
var
    ...
    try
      ...
      begin
        ...
        //begin
        {
          ...
          //frmENCargoPlanEdit.updateFINMaterialsGrid(temp);
...
procedure TfrmENCargoPlanEdit.cbPlanWorkKindChange(Sender: TObject);
...
begin
  ...
  begin
    ...
    if ((isTransport) and (KindCode = 2)) then
    begin
      gbPlanMOL.Visible := true;
      gbPlanWorkItem.Visible := true;
      DisableControls([edtKartiNum]);
...
procedure TfrmENCargoPlanEdit.spbFINExecutorClick(Sender: TObject);
var
   frmFINExecutorTreeShow: TfrmFINExecutorTreeShow;
 //  f : EditENCargoPlanStateFilter;
begin
...
function TfrmENCargoPlanEdit.checkMaterialsBinding_(
...
procedure TfrmENCargoPlanEdit.Shape1MouseDown(Sender: TObject;
...
procedure TfrmENCargoPlanEdit.btnCloseAllClick(Sender: TObject);
var
  TempENPlanWork: ENPlanWorkControllerSoapPort;
...
procedure TfrmENCargoPlanEdit.btnWorkOrderDetailsClick(Sender: TObject);
var
  TempENWorkOrder2ENPlanWork: ENWorkOrder2ENPlanWorkControllerSoapPort;
  i: Integer;
  ENWorkOrder2ENPlanWorkList: ENWorkOrder2ENPlanWorkShortList;
...
procedure TfrmENCargoPlanEdit.actWorkOrderInsertExecute(Sender: TObject);
var
   TempENWorkOrder: ENWorkOrderControllerSoapPort;
   frmENWorkOrderEdit : TfrmENWorkOrderEdit;
   TempENWorkOrder2ENPlanWork : ENWorkOrder2ENPlanWorkControllerSoapPort;
...
procedure TfrmENCargoPlanEdit.actWorkOrderEditExecute(Sender: TObject);
var
   TempENWorkOrder: ENWorkOrderControllerSoapPort;
   frmENWorkOrderEdit : TfrmENWorkOrderEdit;
   TempENWorkOrder2ENPlanWork : ENWorkOrder2ENPlanWorkControllerSoapPort;
...
procedure TfrmENCargoPlanEdit.LoadMOLData(workOrderCode : Integer);
var
  i,j : Integer;
  f : FINMOLDataFilter;
...
procedure TfrmENCargoPlanEdit.LoadWorkOrder;
var
  TempENWorkOrder2ENPlanWork: ENWorkOrder2ENPlanWorkControllerSoapPort;
...
procedure TfrmENCargoPlanEdit.actWorkOrderUpdateExecute(Sender: TObject);
begin
  LoadWorkOrder;
end;

procedure TfrmENCargoPlanEdit.spbFINMolClick(Sender: TObject);
var
 f : FINMolFilter;
...
procedure TfrmENCargoPlanEdit.InitWorkOrderFields;
begin
  DisableControls([edtWorkOrderCode]);
...
procedure TfrmENCargoPlanEdit.btnWorkOrderSaveClick(Sender: TObject);
var
   TempENWorkOrder: ENWorkOrderControllerSoapPort;
   TempENWorkOrder2ENPlanWork : ENWorkOrder2ENPlanWorkControllerSoapPort;
begin
...
procedure TfrmENCargoPlanEdit.btnWorkOrderCancelClick(Sender: TObject);
begin
  ClearWorkOrderFields;
  LoadWorkOrder;
  WorkOrderEditState := dsView;
...
procedure TfrmENCargoPlanEdit.ClearWorkOrderFields;
begin
...
procedure TfrmENCargoPlanEdit.spbFINMechanicClick(Sender: TObject);
var
 f : FINMolFilter;
 frmFINMolShow : TfrmFINMolShow;
...
procedure TfrmENCargoPlanEdit.pcPlanWorkChanging(Sender: TObject;
  var AllowChange: Boolean);
var answer: Integer;
begin
  AllowChange := true;
...
procedure TfrmENCargoPlanEdit.actWorkOrderDeleteExecute(Sender: TObject);
var
   TempENWorkOrder: ENWorkOrderControllerSoapPort;
   frmENWorkOrderEdit : TfrmENWorkOrderEdit;
   TempENWorkOrder2ENPlanWork : ENWorkOrder2ENPlanWorkControllerSoapPort;
...
procedure TfrmENCargoPlanEdit.cbShowAllClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmENCargoPlanEdit.spbPlanMOLClick(Sender: TObject);
var
 f : FINMolFilter;
 frmFINMolShow : TfrmFINMolShow;
...
procedure TfrmENCargoPlanEdit.spbPlanMOLClearClick(Sender: TObject);
var
  ENMOL2PlanWorkObj: ENMOL2PlanWork;
  TempENMOL2PlanWork: ENMOL2PlanWorkControllerSoapPort;
  ENMOL2PlanWorkFilterObj : ENMOL2PlanWorkFilter;
...
procedure TfrmENCargoPlanEdit.actAddToRQOrderExecute(Sender: TObject);
var
  f : RQOrderFilter;
...
procedure TfrmENCargoPlanEdit.actRemoveFromRQOrderExecute(Sender: TObject);
var
  TempRQOrderItem: RQOrderItemControllerSoapPort;
  j : Integer;
begin
...
procedure TfrmENCargoPlanEdit.actMOLDataInsertExecute(Sender: TObject);
Var
  TempFINMolData: FINMolDataControllerSoapPort;
begin
  TempFINMolData := HTTPRIOFINMolData as FINMolDataControllerSoapPort;
...
procedure TfrmENCargoPlanEdit.actMOLDataDeleteExecute(Sender: TObject);
Var
  TempFINMolData: FINMolDataControllerSoapPort;
  objCode : Integer;
begin
...
procedure TfrmENCargoPlanEdit.actMOLDataEditExecute(Sender: TObject);
...
procedure TfrmENCargoPlanEdit.actMOLDataViewExecute(Sender: TObject);
Var TempFINMolData: FINMolDataControllerSoapPort;
begin
 TempFINMolData := HTTPRIOFINMolData as FINMolDataControllerSoapPort;
   try
...
procedure TfrmENCargoPlanEdit.actMOLDataUpdateExecute(Sender: TObject);
begin
  //inherited;
...
procedure TfrmENCargoPlanEdit.spbBindingOverClick(Sender: TObject);
Var
TempENBindingOver: ENBindingOverControllerSoapPort;
...
procedure TfrmENCargoPlanEdit.actExpToExcelExecute(Sender: TObject);
...
procedure TfrmENCargoPlanEdit.actExpToExcel_materialExecute(
  Sender: TObject);
var AppPath, FileName: String;
    OldCursor: TCursor;
begin
...
procedure TfrmENCargoPlanEdit.actExpToExcelDemontajExecute(Sender: TObject);
var AppPath, FileName: String;
    OldCursor: TCursor;
begin
  OldCursor := Screen.Cursor;
...
procedure TfrmENCargoPlanEdit.actExpToExcelHumanExecute(Sender: TObject);
var AppPath, FileName: String;
    OldCursor: TCursor;
begin
  OldCursor := Screen.Cursor;
...
procedure TfrmENCargoPlanEdit.actChangeEstimateItemStatusExecute(
  Sender: TObject);
  var
    TempENEstimateItem: ENEstimateItemControllerSoapPort;
...
procedure TfrmENCargoPlanEdit.actSelectAllExecute(Sender: TObject);
begin
  inherited;
  checkGrid(sgENEstimateItem, 1, true);
end;

procedure TfrmENCargoPlanEdit.actClearAllExecute(Sender: TObject);
begin
  inherited;
  checkGrid(sgENEstimateItem, 1, false);
end;

procedure TfrmENCargoPlanEdit.actChangeEstimateItemStatusPLANEDExecute(
  Sender: TObject);
  var
    TempENEstimateItem: ENEstimateItemControllerSoapPort;
    i , newStatus, eCode : integer;
...
procedure TfrmENCargoPlanEdit.updateMarksGrid(estimateItemCode : Integer; stringGrid : TAdvStringGrid );
var
...
procedure TfrmENCargoPlanEdit.actMoveTOExecute(Sender: TObject);
var
...
procedure TfrmENCargoPlanEdit.actMoveFromExecute(Sender: TObject);
var
  planFull : ENPlanWorkShort;
begin
  inherited;
...
procedure TfrmENCargoPlanEdit.spbEPKardClick(Sender: TObject);
var
...
procedure TfrmENCargoPlanEdit.chbIsFriableClick(Sender: TObject);
begin
  inherited;
  ...
  begin
    edtLength.Text := '';
    edtHeight.Text := '';
    DisableControls([edtLength, edtHeight], True);
  end else
  begin
    DisableControls([edtLength, edtHeight], False);
    DenyBlankValues([edtLength, edtHeight], True);
  end;
