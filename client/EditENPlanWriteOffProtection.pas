unit EditENPlanWriteOffProtection;
...
type
  TfrmENPlanWriteOffProtectionEdit = class(TDialogForm)
  ...
  btnCancel: TButton;
    ...
    miN12: TMenuItem;
    miN13: TMenuItem;
    miInsertPersonal: TMenuItem;
    miInsertSubstation: TMenuItem;
    miInsertBrigade: TMenuItem;
    miN2: TMenuItem;
    miInsertHand: TMenuItem;
    actInsertSubstation: TAction;
    actInsertPersonal: TAction;
    actInsertBrigade: TAction;
  ...
  procedure UpdateGrid(Sender: TObject);
    ...
    procedure actInsertForOldExecute(Sender: TObject);
    procedure actInsertPersonalExecute(Sender: TObject);
    procedure actInsertSubstationExecute(Sender: TObject);
    procedure actInsertBrigadeExecute(Sender: TObject);
  ...
  public
    { Public declarations }
      ...
      addPlanItemsMode, isWriteOffProtection : Boolean;
      //procedure ChangeCaptions;
end;

var
  frmENPlanWriteOffProtectionEdit: TfrmENPlanWriteOffProtectionEdit;
  //ENPlanWorkObj: ENPlanWork;
...
, ShowENElement, ENElementController, ENPlanWorkItemController,
  ...
  ENDepartmentController, ShowENDepartment,
  ...
  TKTechCardController, ShowTKTechCard, FindMaterialForWriteOff,ENEstimateItemStatusController,ENEstimateItemTypeController, 
...
procedure TfrmENPlanWriteOffProtectionEdit.FormShow(Sender: TObject);
var
...
begin
  ...
  if  ((DialogState = dsInsert) and (isWriteOffProtection)) then
  begin
     if ENPlanWorkObj.status = nil then ENPlanWorkObj.status := ENPlanWorkStatus.Create;
     ENPlanWorkObj.status.code := 1;
    ...
    DisableControls([edtTypeName, spbType, edtWorkState,
                     edtENBudgetName, edtResponsibility, spbENBudget, spbResponsibility]);

    ENPlanWorkObj.typeRef := ENPlanWorkTypeRef.Create();
    ENPlanWorkObj.typeRef.code := 18;
    edtTypeName.Text := 'Списання засобів захисту ';
  ...
  begin
    DisableControls([spbResponsibility ,spbENBudget, spbDepartment, spbType
                    ...
                    //, btnWorkOrderSave
                    //, btnWorkOrderCancel
                   // , btnConvertTranzit2Operative
                    ]);

    DisableActions([actInsert, actInsertForOld , actEdit, actDelete, 
                    actWorkOrderInsert, actWorkOrderEdit, actWorkOrderDelete , actInsertSubstation , actInsertPersonal ,actInsertBrigade]);

    HideControls([btnWorkOrderSave, btnWorkOrderCancel]);
    DisableControls([cbShowAll], false);
...

{
  ...
  // HideControls([lblENActNumber, edtENActNumber, spbENAct]);
   if isWriteOffProtection = true then
    begin
    frmENPlanWriteOffProtectionEdit.cbPlanWorkKind.ItemIndex := ENPLANWORKKIND_FACT-1;
    DisableControls([cbPlanWorkKind]);
    cbPlanWorkKindChange(Self);
...
procedure TfrmENPlanWriteOffProtectionEdit.FormClose(Sender: TObject;  var Action: TCloseAction);
   ...
   ENPlanWorkObj.formRef.code := cbENPlanWorkFormName.ItemIndex + 1;
    ...
    begin
      ...
      case element.typeRef.code of
        ...
        7 : ENPlanWorkObj.code := TempENPlanWork.addByByt(ENPlanWorkObj);
        8 : ENPlanWorkObj.code := TempENPlanWork.addByProm(ENPlanWorkObj);
        ...
        23 : ENPlanWorkObj.code := TempENPlanWork.addByServicesObject(ENPlanWorkObj);
        ...
        if ((element.typeRef.code = EN_WRITING_NO_OBJECT) and  (ENPlanWorkObj.typeRef.code = 18)) then
           ENPlanWorkObj.code := TempENPlanWork.addByWritingOffProtection(ENPlanWorkObj)
        else    
        begin
      ...
      if frmENPlanWriteOffProtectionEdit.molCode <> '' then
      begin
        TempENMOL2PlanWork :=  HTTPRIOENMOL2PlanWork as ENMOL2PlanWorkControllerSoapPort;
        ...
        ENMOL2PlanWorkObj.molCode := frmENPlanWriteOffProtectionEdit.molCode;
        ENMOL2PlanWorkObj.molName := frmENPlanWriteOffProtectionEdit.molName;
...
procedure TfrmENPlanWriteOffProtectionEdit.FormCreate(Sender: TObject);
begin
  inherited;
...
procedure TfrmENPlanWriteOffProtectionEdit.spbENPlanWorkStatusClick(Sender : TObject);
var
...
procedure TfrmENPlanWriteOffProtectionEdit.spbENElementClick(Sender: TObject);
var
...
procedure TfrmENPlanWriteOffProtectionEdit.updateFINMaterialsGrid(estimateItemCode : Integer; stringGrid : TAdvStringGrid );
var
...
procedure TfrmENPlanWriteOffProtectionEdit.pcPlanWorkChange(Sender: TObject);
var
  TempENPlanWorkItem: ENPlanWorkItemControllerSoapPort;
...
begin
  ...
  begin
    actInsert.Caption := 'Добавить с персонала';

    DisableActions([actInsert, actInsertForOld ,   actEdit, actDelete, actFilter, actNoFilter], not (DialogState in [dsEdit, dsInsert]) );
...
//      HideControls([grpFINMaterials],false);
//      grpFINMaterials.Visible:= True;
//      grpFINMaterials.Align := alClient;
      pnl1.Visible := True;
      pnl1.Align := alClient;
      end
    else
...
procedure TfrmENPlanWriteOffProtectionEdit.actInsertExecute(Sender: TObject);
    Var 
     TempENEstimateItem: ENEstimateItemControllerSoapPort;
begin
  ...
  begin
    ...
    try
      ...
      try
        ...
        frmENEstimateItemEdit.DisableControls([frmENEstimateItemEdit.cbENEstimateItemKind, frmENEstimateItemEdit.chbIsCreateObject, frmENEstimateItemEdit.lblPlanItem  , frmENEstimateItemEdit.edtPlanItem    , frmENEstimateItemEdit.spbPlanItem]);
        frmENEstimateItemEdit.HideControls([frmENEstimateItemEdit.lblCountGen, frmENEstimateItemEdit.edtCountGen, frmENEstimateItemEdit.chbIsCreateObject]);
...
procedure TfrmENPlanWriteOffProtectionEdit.UpdateGrid(Sender: TObject);
Var i, j: Integer;
begin
...
procedure TfrmENPlanWriteOffProtectionEdit.actEditExecute(Sender: TObject);
...
///--------------------------------------------------------------------------------
   // если рдактируем материалы и строка добавлена вручную для списания старых остаттков то откроем обычную форму редактирования естимейта
   // estimateitemstatus = 6  значит сформирована строка из приходов по енерджинет
   // estimateitemstatus = 3  значит строка естимейта добавлялась вручную на план для списания старых материалов
...
begin
  ...
  begin
    ...
    try
     ...
     if ENEstimateItemObj.typeRef.code <> ENESTIMATEITEMTYPE_AUTO then   // открываем форму для естимейта созданного вручную.
       begin
        ...
        if ENEstimateItemObj.typeRef.code = ENESTIMATEITEMTYPE_AUTO then  // открываем форму под автоматически сформированные строки
      begin
       TempENWorkOrder := HTTPRIOENWorkOrder as ENWorkOrderControllerSoapPort;

         wofilter := ENWorkOrderFilter.Create;
         SetNullIntProps(wofilter);
         SetNullXSProps(wofilter);
         wofilter.conditionSQL := ' ENWORKORDER.CODE in (select distinct(w2p.workordercode)  from enworkorder2enplanwork w2p ' +
                                  ' where w2p.plancode = ' + IntToStr(ENPlanWorkObj.code) +  ' ) ';

         woList:=  TempENWorkOrder.getScrollableFilteredList(wofilter, 0,-1);
         if woList.totalCount > 0 then
            if woList.list[0].finMolCode <> '' then
              begin
               masterMOLCode := woList.list[0].finMolCode;
               masterMOLName := woList.list[0].finMolName;
              end
            else
            begin
             Application.MessageBox(PChar('Не заведений МОЛ в наряд завданні!'), PChar('Увага'), MB_ICONWARNING);
         ...
         begin
           Application.MessageBox(PChar('Заведіть наряд завдання!'), PChar('Увага'), MB_ICONWARNING);
           pcPlanWork.ActivePage:= tsWorkOrder;
           UpdateGrid(Sender);
           exit;
          end;    
        ...
        begin
          frmFindMaterialForWriteOff := TfrmFindMaterialForWriteOff.Create(Application, dsView);
          try
             // выбрать МОЛ мастера с которого будем искать материалы для списания
             frmFindMaterialForWriteOff.masterMOLCode := masterMOLCode;
             frmFindMaterialForWriteOff.masterMOLName := masterMOLName;
             frmFindMaterialForWriteOff.btnSave.Caption := 'OK';
             frmFindMaterialForWriteOff.estimateFromNewplan := StrToInt( sgENEstimateItem.Cells[0, sgENEstimateItem.Row ]);

             // определим тип элемента (работники, бригада, подстанция) - что б определить какой лист выполнять при просмотре строк
             // c которых сформирована просматриваемая строка .
               ENElTypeFilter:= ENElementTypeFilter.Create;
              SetNullIntProps(ENElTypeFilter);
              ENElTypeFilter.conditionSQL := ' code in ( Select el.typerefcode From enestimateitem2nstmttm qq , enestimateitem ii , enplanwork p , enelement el ' +
                                ' Where qq.estimateitemoutrefcode = ' + sgENEstimateItem.Cells[0, sgENEstimateItem.Row ] +
                                ' and qq.estimateiteminrefcode = ii.code ' +
             ...
             ENElementTypeList := TempENElementType.getScrollableFilteredList(ENElTypeFilter,0,-1);
             if ENElementTypeList.totalCount <> 0 then
                if ENElementTypeList.list[0].code = EN_SUBSTATION150 then
                   frmFindMaterialForWriteOff.methodInsert := 3
                else if ENElementTypeList.list[0].code = EN_SIZ_OBJECT then
                   frmFindMaterialForWriteOff.methodInsert := 1
                else if ENElementTypeList.list[0].code = EN_SZ_BRIGADE then
                   frmFindMaterialForWriteOff.methodInsert := 2;



             if frmFindMaterialForWriteOff.ShowModal = mrOK then
             begin
                 UpdateGrid(Self);
             end;
          finally

              frmFindMaterialForWriteOff.Free;
              frmFindMaterialForWriteOff := nil;
          end;
        end;
      end;
...
procedure TfrmENPlanWriteOffProtectionEdit.actUpdateExecute(Sender: TObject);
begin
  inherited;
...
procedure TfrmENPlanWriteOffProtectionEdit.btnPlanReportClick(Sender: TObject);
var
...
procedure TfrmENPlanWriteOffProtectionEdit.actDeleteExecute(Sender: TObject);
Var TempENPlanWorkItem: ENPlanWorkItemControllerSoapPort;
  ObjCode: Integer;
  TempENEstimateItem: ENEstimateItemControllerSoapPort;
  eType : integer;
...
//     end;
     ...
     if  Integer(sgENEstimateItem.Objects[0,sgENEstimateItem.Row]) in [ENESTIMATEITEMTYPE_MANUAL_BY_PLAN , ENESTIMATEITEMTYPE_AUTO] then //такие удалять из РАБОТЫ .., ENESTIMATEITEMTYPE_MANUAL_BY_PLANITEM]  then
     begin
       TempENEstimateItem := HTTPRIOENEstimateItem as ENEstimateItemControllerSoapPort;
         try
        ...
        begin
            ...
            TempENEstimateItem.removeForWriteOff(ObjCode);
            UpdateGrid(Sender);
        end;
     end
     else
...
procedure TfrmENPlanWriteOffProtectionEdit.sgENPlanCorrectHistoryDblClick(
  Sender: TObject);
  var
  frmViewOldPlan : TfrmENPlanWriteOffProtectionEdit;
  TempENPlanCorrectHistory : ENPlanCorrectHistoryControllerSoapPort;
  TempENPlanWork: ENPlanWorkControllerSoapPort;
...
begin
   ...
   frmViewOldPlan := TfrmENPlanWriteOffProtectionEdit.Create(Application, dsView);
...
procedure TfrmENPlanWriteOffProtectionEdit.spbENBudgetClick(Sender: TObject);
var
...
procedure TfrmENPlanWriteOffProtectionEdit.spbResponsibilityClick(Sender: TObject);
var
   frmENDepartmentShow: TfrmENDepartmentShow;
   f : ENDepartmentFilter;
begin
...
procedure TfrmENPlanWriteOffProtectionEdit.spbDepartmentClick(Sender: TObject);
var
   frmENDepartmentShow: TfrmENDepartmentShow;
   f : ENDepartmentFilter;
begin
...
procedure TfrmENPlanWriteOffProtectionEdit.spbTypeClick(Sender: TObject);
var
...
begin
   ...
   try
      with frmENPlanWorkTypeShow do begin
        DisableActions([ actEdit, actInsert , actInsertForOld ,  actDelete]);
        if ShowModal = mrOk then
        begin
            try
...
procedure TfrmENPlanWriteOffProtectionEdit.edtDateStartClick(Sender: TObject);
begin
//if edtDateStart.Checked then
//begin
...
procedure TfrmENPlanWriteOffProtectionEdit.actMaterialBindingExecute(Sender: TObject);
begin
...
procedure TfrmENPlanWriteOffProtectionEdit.PopupMenu1Popup(Sender: TObject);
begin
...
procedure TfrmENPlanWriteOffProtectionEdit.edtDateStartChange(Sender: TObject);
begin
...
procedure TfrmENPlanWriteOffProtectionEdit.spbENPlanWorkStateClick(Sender: TObject);
var
   ...
   if ENPlanWorkObj.elementRef <> nil then
     ...
     begin
       ...
       if (e.typeRef.code <> EN_SUBSTATION150) and
         ...
         f.conditionSQL := f.conditionSQL + '  enplanworkstate.code <> ' + IntToStr(ENPLANWORKSTATE_CURRENTREPAIR);
   end;

   // если план на списание средств защиты тогда тип акта доступен только (Списання МБП)  и (Списання МНМА)
   if isWriteOffProtection then
     if f.conditionSQL <> '' then
        f.conditionSQL := f.conditionSQL + '  and enplanworkstate.code in (15,23) '
     else
        f.conditionSQL := ' enplanworkstate.code in (15,23) ';

   frmENPlanWorkStateShow:=TfrmENPlanWorkStateShow.Create(Application,fmNormal,f);
   try
      with frmENPlanWorkStateShow do begin
        DisableActions([ actEdit, actInsert, actInsertForOld ,  actDelete, actNoFilter, actFilter ]);
        if ShowModal = mrOk then
        begin
            try
...
procedure TfrmENPlanWriteOffProtectionEdit.spbENActClick(Sender: TObject);
var
  TempENAct: ENActControllerSoapPort;
  i: Integer;
...
procedure TfrmENPlanWriteOffProtectionEdit.sgENEstimateItemClick(Sender: TObject);
var
...
begin
  ...
  begin
    ...
    if 1 = TK_ACCOUNTINGTYPE_TMC then
      updateFINMaterialsGrid(j, sgFINMaterials);
...
procedure TfrmENPlanWriteOffProtectionEdit.actMaterialBindingToFINExecute(
  Sender: TObject);
var
   ...
   try
    ...
    try
      ...
      begin
        ...
        //frmFINMaterialsDataEdit.isWriteOffProtection := True;
      end;
    except
      on EConvertError do Exit;
    end;
...
procedure TfrmENPlanWriteOffProtectionEdit.cbPlanWorkKindChange(Sender: TObject);
...
procedure TfrmENPlanWriteOffProtectionEdit.spbFINExecutorClick(Sender: TObject);
var
...
begin
   ...
   try
      with frmFINExecutorTreeShow do begin
        DisableActions([ actEdit, actInsert , actInsertForOld ]);
        if ShowModal = mrOk then
        begin
            try
...
function TfrmENPlanWriteOffProtectionEdit.checkMaterialsBinding_(
...
procedure TfrmENPlanWriteOffProtectionEdit.Shape1MouseDown(Sender: TObject;
...
procedure TfrmENPlanWriteOffProtectionEdit.btnCloseAllClick(Sender: TObject);
var
  TempENPlanWork: ENPlanWorkControllerSoapPort;
...
procedure TfrmENPlanWriteOffProtectionEdit.btnWorkOrderDetailsClick(Sender: TObject);
var
  TempENWorkOrder2ENPlanWork: ENWorkOrder2ENPlanWorkControllerSoapPort;
  i: Integer;
  ENWorkOrder2ENPlanWorkList: ENWorkOrder2ENPlanWorkShortList;
...
procedure TfrmENPlanWriteOffProtectionEdit.actWorkOrderInsertExecute(Sender: TObject);
var
   TempENWorkOrder: ENWorkOrderControllerSoapPort;
   frmENWorkOrderEdit : TfrmENWorkOrderEdit;
   TempENWorkOrder2ENPlanWork : ENWorkOrder2ENPlanWorkControllerSoapPort;
...
procedure TfrmENPlanWriteOffProtectionEdit.actWorkOrderEditExecute(Sender: TObject);
var
   TempENWorkOrder: ENWorkOrderControllerSoapPort;
   frmENWorkOrderEdit : TfrmENWorkOrderEdit;
   TempENWorkOrder2ENPlanWork : ENWorkOrder2ENPlanWorkControllerSoapPort;
...
procedure TfrmENPlanWriteOffProtectionEdit.LoadMOLData(workOrderCode : Integer);
var
  i,j : Integer;
  f : FINMOLDataFilter;
...
procedure TfrmENPlanWriteOffProtectionEdit.LoadWorkOrder;
var
  TempENWorkOrder2ENPlanWork: ENWorkOrder2ENPlanWorkControllerSoapPort;
...
procedure TfrmENPlanWriteOffProtectionEdit.actWorkOrderUpdateExecute(Sender: TObject);
begin
  LoadWorkOrder;
end;

procedure TfrmENPlanWriteOffProtectionEdit.spbFINMolClick(Sender: TObject);
var
 f : FINMolFilter;
...
procedure TfrmENPlanWriteOffProtectionEdit.InitWorkOrderFields;
begin
  DisableControls([edtWorkOrderCode]);
...
procedure TfrmENPlanWriteOffProtectionEdit.btnWorkOrderSaveClick(Sender: TObject);
var
   TempENWorkOrder: ENWorkOrderControllerSoapPort;
   TempENWorkOrder2ENPlanWork : ENWorkOrder2ENPlanWorkControllerSoapPort;
begin
...
procedure TfrmENPlanWriteOffProtectionEdit.btnWorkOrderCancelClick(Sender: TObject);
begin
  ClearWorkOrderFields;
  LoadWorkOrder;
  WorkOrderEditState := dsView;
...
procedure TfrmENPlanWriteOffProtectionEdit.ClearWorkOrderFields;
begin
...
procedure TfrmENPlanWriteOffProtectionEdit.spbFINMechanicClick(Sender: TObject);
var
 f : FINMolFilter;
 frmFINMolShow : TfrmFINMolShow;
...
procedure TfrmENPlanWriteOffProtectionEdit.pcPlanWorkChanging(Sender: TObject;
  var AllowChange: Boolean);
var answer: Integer;
begin
  AllowChange := true;
...
procedure TfrmENPlanWriteOffProtectionEdit.actWorkOrderDeleteExecute(Sender: TObject);
var
   TempENWorkOrder: ENWorkOrderControllerSoapPort;
   frmENWorkOrderEdit : TfrmENWorkOrderEdit;
   TempENWorkOrder2ENPlanWork : ENWorkOrder2ENPlanWorkControllerSoapPort;
...
procedure TfrmENPlanWriteOffProtectionEdit.cbShowAllClick(Sender: TObject);
begin
  inherited;
...
procedure TfrmENPlanWriteOffProtectionEdit.spbPlanMOLClick(Sender: TObject);
var
 f : FINMolFilter;
 frmFINMolShow : TfrmFINMolShow;
...
procedure TfrmENPlanWriteOffProtectionEdit.spbPlanMOLClearClick(Sender: TObject);
var
  ENMOL2PlanWorkObj: ENMOL2PlanWork;
  TempENMOL2PlanWork: ENMOL2PlanWorkControllerSoapPort;
  ENMOL2PlanWorkFilterObj : ENMOL2PlanWorkFilter;
...
procedure TfrmENPlanWriteOffProtectionEdit.actAddToRQOrderExecute(Sender: TObject);
var
  f : RQOrderFilter;
...
procedure TfrmENPlanWriteOffProtectionEdit.actRemoveFromRQOrderExecute(Sender: TObject);
var
  TempRQOrderItem: RQOrderItemControllerSoapPort;
  j : Integer;
begin
...
procedure TfrmENPlanWriteOffProtectionEdit.actMOLDataInsertExecute(Sender: TObject);
Var
  TempFINMolData: FINMolDataControllerSoapPort;
begin
  TempFINMolData := HTTPRIOFINMolData as FINMolDataControllerSoapPort;
...
procedure TfrmENPlanWriteOffProtectionEdit.actMOLDataDeleteExecute(Sender: TObject);
Var
  TempFINMolData: FINMolDataControllerSoapPort;
  objCode : Integer;
begin
...
procedure TfrmENPlanWriteOffProtectionEdit.actMOLDataEditExecute(Sender: TObject);
...
procedure TfrmENPlanWriteOffProtectionEdit.actMOLDataViewExecute(Sender: TObject);
Var TempFINMolData: FINMolDataControllerSoapPort;
begin
 TempFINMolData := HTTPRIOFINMolData as FINMolDataControllerSoapPort;
   try
...
procedure TfrmENPlanWriteOffProtectionEdit.actMOLDataUpdateExecute(Sender: TObject);
begin
  //inherited;
...
procedure TfrmENPlanWriteOffProtectionEdit.spbBindingOverClick(Sender: TObject);
Var
TempENBindingOver: ENBindingOverControllerSoapPort;
...
procedure TfrmENPlanWriteOffProtectionEdit.actExpToExcelExecute(Sender: TObject);
...
procedure TfrmENPlanWriteOffProtectionEdit.actExpToExcel_materialExecute(
  Sender: TObject);
var AppPath, FileName: String;
    OldCursor: TCursor;
begin
...
procedure TfrmENPlanWriteOffProtectionEdit.actExpToExcelDemontajExecute(Sender: TObject);
var AppPath, FileName: String;
    OldCursor: TCursor;
begin
  OldCursor := Screen.Cursor;
...
procedure TfrmENPlanWriteOffProtectionEdit.actExpToExcelHumanExecute(Sender: TObject);
var AppPath, FileName: String;
    OldCursor: TCursor;
begin
  OldCursor := Screen.Cursor;
...
procedure TfrmENPlanWriteOffProtectionEdit.actChangeEstimateItemStatusExecute(
  Sender: TObject);
  var
    TempENEstimateItem: ENEstimateItemControllerSoapPort;
...
procedure TfrmENPlanWriteOffProtectionEdit.actSelectAllExecute(Sender: TObject);
begin
  inherited;
  checkGrid(sgENEstimateItem, 1, true);
end;

procedure TfrmENPlanWriteOffProtectionEdit.actClearAllExecute(Sender: TObject);
begin
  inherited;
  checkGrid(sgENEstimateItem, 1, false);
end;

procedure TfrmENPlanWriteOffProtectionEdit.actChangeEstimateItemStatusPLANEDExecute(
  Sender: TObject);
  var
    TempENEstimateItem: ENEstimateItemControllerSoapPort;
    i , newStatus, eCode : integer;
...
procedure TfrmENPlanWriteOffProtectionEdit.updateMarksGrid(estimateItemCode : Integer; stringGrid : TAdvStringGrid );
var
...
procedure TfrmENPlanWriteOffProtectionEdit.actMoveTOExecute(Sender: TObject);
var
...
procedure TfrmENPlanWriteOffProtectionEdit.actMoveFromExecute(Sender: TObject);
var
  planFull : ENPlanWorkShort;
begin
  inherited;
...
procedure TfrmENPlanWriteOffProtectionEdit.spbEPKardClick(Sender: TObject);
var
...
begin
   ...
   if isWriteOffProtection then //frmKartiShow.isWriteOffProtection := true;
...
procedure TfrmENPlanWriteOffProtectionEdit.btnConvertTranzit2OperativeClick(
  Sender: TObject);
...
begin
   TempENWorkOrder := HTTPRIOENWorkOrder as ENWorkOrderControllerSoapPort;
    ...
    begin
      frmFindMaterialForWriteOff := TfrmFindMaterialForWriteOff.Create(Application, dsInsert);
      try
         // выбрать МОЛ мастера с которого будем искать материалы для списания
         frmFindMaterialForWriteOff.masterMOLCode := masterMOLCode;
         frmFindMaterialForWriteOff.masterMOLName := masterMOLName;
...
procedure TfrmENPlanWriteOffProtectionEdit.actInsertForOldExecute(
  Sender: TObject);
  var
  TempENEstimateItem: ENEstimateItemControllerSoapPort;
begin
  ...
  begin
    ...
    ENEstimateItemObj.statusRef.code :=  ENESTIMATEITEMSTATUS_LOADED;
...
procedure TfrmENPlanWriteOffProtectionEdit.actInsertPersonalExecute(
  Sender: TObject);
var
     TempENWorkOrder: ENWorkOrderControllerSoapPort;
     woList : ENWorkOrderShortList;
   ...
   TempENWorkOrder := HTTPRIOENWorkOrder as ENWorkOrderControllerSoapPort;
    ...
    begin
      ...
      try
         ...
         frmFindMaterialForWriteOff.masterMOLCode := masterMOLCode;
         frmFindMaterialForWriteOff.masterMOLName := masterMOLName;
         frmFindMaterialForWriteOff.methodInsert:= 1; // c персонала 
...
procedure TfrmENPlanWriteOffProtectionEdit.actInsertSubstationExecute(
  Sender: TObject);
var
     TempENWorkOrder: ENWorkOrderControllerSoapPort;
     woList : ENWorkOrderShortList;
   ...
   TempENWorkOrder := HTTPRIOENWorkOrder as ENWorkOrderControllerSoapPort;
    ...
    begin
      ...
      try
         ...
         frmFindMaterialForWriteOff.masterMOLCode := masterMOLCode;
         frmFindMaterialForWriteOff.masterMOLName := masterMOLName;
         frmFindMaterialForWriteOff.methodInsert:= 3; // c подстанций  
...
procedure TfrmENPlanWriteOffProtectionEdit.actInsertBrigadeExecute(
  Sender: TObject);
var
     TempENWorkOrder: ENWorkOrderControllerSoapPort;
     woList : ENWorkOrderShortList;
   ...
   TempENWorkOrder := HTTPRIOENWorkOrder as ENWorkOrderControllerSoapPort;
    ...
    begin
      ...
      try
         ...
         frmFindMaterialForWriteOff.masterMOLCode := masterMOLCode;
         frmFindMaterialForWriteOff.masterMOLName := masterMOLName;
         frmFindMaterialForWriteOff.methodInsert:= 2; // c бригад 
