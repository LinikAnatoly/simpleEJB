unit ENFuseController;

interface

uses InvokeRegistry, SOAPHTTPClient, Types, XSBuiltIns, EnergyProController, EnergyProController2 
   ,ENFuseTypeController 
   ,ENElementController 
   ,TKMaterialsController 
   ,ENHighVoltageSellController 
   ,ENBranchController 
   ,ENLowVoltBoardController 
   ,ENPanelController 
;

type

  // ************************************************************************ //
  // The following types, referred to in the WSDL document are not being represented
  // in this file. They are either aliases[@] of other types represented or were referred
  // to but never[!] declared in the document. The types from the latter category
  // typically map to predefined/known XML or Borland types; however, they could also 
  // indicate incorrect WSDL documents that failed to declare or import a schema type.
  // ************************************************************************ //
  // !:int             - "http://www.w3.org/2001/XMLSchema"
  // !:string          - "http://www.w3.org/2001/XMLSchema"
  // !:decimal         - "http://www.w3.org/2001/XMLSchema"
  // !:date            - "http://www.w3.org/2001/XMLSchema"
  // !:long            - "http://www.w3.org/2001/XMLSchema"
  // !:dateTime        - "http://www.w3.org/2001/XMLSchema"
  // !:EPCalculatorShortList - "http://ksoe.org/EnergyproControllerService/type/"
  // !:EPCalculatorFilter - "http://ksoe.org/EnergyproControllerService/type/"

  ENFuse            = class;                 { "http://ksoe.org/EnergyproControllerService/type/" }



  // ************************************************************************ //
  // Namespace : http://ksoe.org/EnergyproControllerService/type/
  // ************************************************************************ //
  ENFuseRef = class(TRemotable)
  private
    Fcode: Integer;
  published
    property code: Integer read Fcode write Fcode;
  end;


  // ************************************************************************ //
  // Namespace : http://ksoe.org/EnergyproControllerService/type/
  // ************************************************************************ //
  ENFuse = class(TRemotable)
  private
    Fcode : Integer; 
    Fname : WideString;
    FcurrentFuse : TXSDecimal;
    Fmodify_time : Int64;
//???
    FfuseType : ENFuseType;
//???
    Felement : ENElement;
//???
    FmaterialRef : TKMaterialsRef;
//???
    FhighVoltageSell : ENHighVoltageSell;
//???
    Fbranch : ENBranch;
//???
    FlvbRef : ENLowVoltBoardRef;
//???
    Fpanel : ENPanelRef;
  public
    destructor Destroy; override;
  published
    property  code : Integer read Fcode write Fcode; 
    property name : WideString read Fname write Fname;
    property currentFuse : TXSDecimal read FcurrentFuse write FcurrentFuse; 
    property modify_time : Int64 read Fmodify_time write Fmodify_time; 
    property fuseType : ENFuseType read FfuseType write FfuseType; 
    property element : ENElement read Felement write Felement; 
    property materialRef : TKMaterialsRef read FmaterialRef write FmaterialRef; 
    property highVoltageSell : ENHighVoltageSell read FhighVoltageSell write FhighVoltageSell; 
    property branch : ENBranch read Fbranch write Fbranch; 
    property lvbRef : ENLowVoltBoardRef read FlvbRef write FlvbRef; 
    property panel : ENPanelRef read Fpanel write Fpanel; 
  end;
  
{
  ENFuseFilter = class(TRemotable)
  private
    FconditionSQL: WideString;
    ForderBySQL: WideString;
    Fcode : Integer; 
    Fname : WideString;
    FcurrentFuse : TXSDecimal;
    Fmodify_time : Int64;
//???
    FfuseType : ENFuseType;
//???
    Felement : ENElement;
//???
    FmaterialRef : TKMaterialsRef;
//???
    FhighVoltageSell : ENHighVoltageSell;
//???
    Fbranch : ENBranch;
//???
    FlvbRef : ENLowVoltBoardRef;
//???
    Fpanel : ENPanelRef;
  public
    destructor Destroy; override;
  published
    property conditionSQL: WideString read FconditionSQL write FconditionSQL;
    property orderBySQL: WideString read ForderBySQL write ForderBySQL;
    property  code : Integer read Fcode write Fcode; 
    property name : WideString read Fname write Fname;
    property currentFuse : TXSDecimal read FcurrentFuse write FcurrentFuse; 
    property modify_time : Int64 read Fmodify_time write Fmodify_time; 
    property fuseType : ENFuseType read FfuseType write FfuseType; 
    property element : ENElement read Felement write Felement; 
    property materialRef : TKMaterialsRef read FmaterialRef write FmaterialRef; 
    property highVoltageSell : ENHighVoltageSell read FhighVoltageSell write FhighVoltageSell; 
    property branch : ENBranch read Fbranch write Fbranch; 
    property lvbRef : ENLowVoltBoardRef read FlvbRef write FlvbRef; 
    property panel : ENPanelRef read Fpanel write Fpanel; 
  end;
}

  ENFuseFilter = class(ENFuse)
  private
    FconditionSQL: WideString;
    ForderBySQL: WideString;
  public
    destructor Destroy; override;
  published
    property conditionSQL: WideString read FconditionSQL write FconditionSQL;
    property orderBySQL: WideString read ForderBySQL write ForderBySQL;
  end;
  
  ENFuseShort = class(TRemotable)
  private
    Fcode: Integer;
    Fname : WideString; 
    FcurrentFuse: TXSDecimal;
    FfuseTypeCode: Integer; 
    FfuseTypeName: WideString;
    FelementCode: Integer; 
    FmaterialRefCode: Integer; 
    FmaterialRefName: WideString;
    FmaterialRefCost: TXSDecimal;
    FmaterialRefDeliveryDate: Integer; 
    FmaterialRefNumkatalog: WideString;
    FmaterialRefIdentid: WideString;
    Fsubstation04Code: Integer;
    Fsubstation04Name: WideString;
    FhighVoltageSellCode: Integer; 
    FhighVoltageSellName: WideString;
    FhighVoltageSellNumberGen: WideString;
    FbranchCode: Integer; 
    FbranchName: WideString;
    FbranchBasicConsumer: WideString;
    FbranchCurrentAdmis : TXSDecimal;
    FbranchNumberGen: WideString;
    FpanelCode: Integer;
    FpanelName: WideString;
    FlvbRefCode : Integer; 
    FlvbRefName : WideString;
    FtransformerCode: Integer;
    FtransformerInvNumber: WideString;
    FtransformerName: WideString;
    FtransformerNominalPower: TXSDecimal;
  public
    destructor Destroy; override;
  published
    property code: Integer read Fcode write Fcode;
    property name : WideString read Fname write Fname;
    property currentFuse: TXSDecimal read FcurrentFuse write FcurrentFuse;
    property fuseTypeCode: Integer read FfuseTypeCode write FfuseTypeCode; 
    property fuseTypeName: WideString read FfuseTypeName write FfuseTypeName; 
    property elementCode: Integer read FelementCode write FelementCode; 
    property materialRefCode: Integer read FmaterialRefCode write FmaterialRefCode; 
    property materialRefName: WideString read FmaterialRefName write FmaterialRefName; 
    property materialRefCost: TXSDecimal read FmaterialRefCost write FmaterialRefCost; 
    property materialRefDeliveryDate: Integer read FmaterialRefDeliveryDate write FmaterialRefDeliveryDate; 
    property materialRefNumkatalog: WideString read FmaterialRefNumkatalog write FmaterialRefNumkatalog;
    property materialRefIdentid: WideString read FmaterialRefIdentid write FmaterialRefIdentid;
    property substation04Code: Integer read Fsubstation04Code write Fsubstation04Code;
    property substation04Name: WideString read Fsubstation04Name write Fsubstation04Name; 
    property highVoltageSellCode: Integer read FhighVoltageSellCode write FhighVoltageSellCode;
    property highVoltageSellName: WideString read FhighVoltageSellName write FhighVoltageSellName; 
    property highVoltageSellNumberGen: WideString read FhighVoltageSellNumberGen write FhighVoltageSellNumberGen; 
    property branchCode: Integer read FbranchCode write FbranchCode;
    property branchName: WideString read FbranchName write FbranchName;
    property branchBasicConsumer: WideString read FbranchBasicConsumer write FbranchBasicConsumer;
    property branchNumberGen: WideString read FbranchNumberGen write FbranchNumberGen;
    property branchCurrentAdmis : TXSDecimal read FbranchCurrentAdmis write FbranchCurrentAdmis; 
    property panelCode: Integer read FpanelCode write FpanelCode;
    property panelName: WideString read FpanelName write FpanelName;
    property lvbRefCode : Integer read FlvbRefCode write FlvbRefCode; 
    property lvbRefName : WideString read FlvbRefName write FlvbRefName;
    property transformerCode: Integer read FtransformerCode write FtransformerCode;
    property transformerInvNumber: WideString read FtransformerInvNumber write FtransformerInvNumber;
    property transformerName: WideString read FtransformerName write FtransformerName;
    property transformerNominalPower: TXSDecimal read FtransformerNominalPower write FtransformerNominalPower;
  end;

  ArrayOfENFuseShort = array of ENFuseShort;  // { "http://ksoe.org/EnergyproControllerService/type/" }

  ENFuseShortList = class(TRemotable)
  private
    FtotalCount: Integer;
    Flist: ArrayOfENFuseShort;
  public
  destructor Destroy; override;
  published
    property totalCount: Integer read FtotalCount write FtotalCount;
    property list: ArrayOfENFuseShort read Flist write Flist;
  end;

  ArrayOfInteger = array of Integer;

  // ************************************************************************ //
  // Namespace : http://ksoe.org/ENFuseController/message/
  // soapAction: http://ksoe.org/ENFuseController/action/ENFuseController.%operationName%
  // transport : http://schemas.xmlsoap.org/soap/http
  // style     : rpc
  // binding   : ENFuseControllerSoapBinding
  // service   : EnergyproControllerService
  // port      : ENFuseController
  // URL       : http://soap.ksoe.com.ua/energypro
  // ************************************************************************ //


  ENFuseControllerSoapPort = interface(IInvokable)
  ['{d3b0d3b0-d3b0-d3b0-d3b0-d3b0d3b0d3b0}']
    function  add(const aENFuse: ENFuse): Integer; stdcall;
    procedure remove(const anObjectCode: Integer); stdcall;
    procedure save(const aENFuse: ENFuse); stdcall;
    function  getObject(const anObjectCode: Integer): ENFuse; stdcall;
    function  getList: ENFuseShortList; stdcall;
    function  getFilteredList(const aENFuseFilter: ENFuseFilter): ENFuseShortList; stdcall;
    function  getScrollableList(const aFromPosition: Integer; const aQuantity: Integer): ENFuseShortList; stdcall;
    function  getScrollableFilteredList(const aENFuseFilter: ENFuseFilter; const aFromPosition: Integer; const aQuantity: Integer): ENFuseShortList; stdcall;
    function  getScrollableListByCondition(const aCondition: WideString; const aFromPosition: Integer; const aQuantity: Integer): ENFuseShortList; stdcall;
    function  getScrollableFilteredCodeArray(const aENFuseFilter: ENFuseFilter; const aFromPosition: Integer; const aQuantity: Integer): ArrayOfInteger; stdcall;

  end; 


implementation

  destructor ENFuse.Destroy;
  begin
    if Assigned(FcurrentFuse) then
      currentFuse.Free;
    if Assigned(FfuseType) then
      fuseType.Free;
    if Assigned(Felement) then
      element.Free;
    if Assigned(FmaterialRef) then
      materialRef.Free;
    if Assigned(FhighVoltageSell) then
      highVoltageSell.Free;
    if Assigned(Fbranch) then
      branch.Free;
    if Assigned(FlvbRef) then
      lvbRef.Free;
    if Assigned(Fpanel) then
      panel.Free;
    inherited Destroy;
  end;

{  
  destructor ENFuseFilter.Destroy;
  begin
    if Assigned(FcurrentFuse) then
      currentFuse.Free;
    if Assigned(FfuseType) then
      fuseType.Free;
    if Assigned(Felement) then
      element.Free;
    if Assigned(FmaterialRef) then
      materialRef.Free;
    if Assigned(FhighVoltageSell) then
      highVoltageSell.Free;
    if Assigned(Fbranch) then
      branch.Free;
    if Assigned(FlvbRef) then
      lvbRef.Free;
    if Assigned(Fpanel) then
      panel.Free;
    inherited Destroy;
  end; 
}

  destructor ENFuseFilter.Destroy;
  begin
    inherited Destroy;
  end; 

  destructor ENFuseShort.Destroy;
  begin
    if Assigned(FcurrentFuse) then
      currentFuse.Free;
    if Assigned(FmaterialRefCost) then
      materialRefCost.Free;
    if Assigned(FbranchCurrentAdmis) then
      branchCurrentAdmis.Free;
    inherited Destroy;
  end; 
  
  destructor ENFuseShortList.Destroy;
  var
    I: Integer;
  begin
    for I := 0 to Length(Flist)-1 do
     if Assigned(Flist[I]) then
       Flist[I].Free;
     SetLength(Flist, 0);
     inherited Destroy;
  end;



initialization

  RemClassRegistry.RegisterXSClass(ENFuse, 'http://ksoe.org/EnergyproControllerService/type/', 'ENFuse');
  RemClassRegistry.RegisterXSClass(ENFuseRef, 'http://ksoe.org/EnergyproControllerService/type/', 'ENFuseRef');
  RemClassRegistry.RegisterXSClass(ENFuseFilter, 'http://ksoe.org/EnergyproControllerService/type/', 'ENFuseFilter');
  RemClassRegistry.RegisterXSClass(ENFuseShort, 'http://ksoe.org/EnergyproControllerService/type/', 'ENFuseShort');
  RemClassRegistry.RegisterXSClass(ENFuseShortList, 'http://ksoe.org/EnergyproControllerService/type/', 'ENFuseShortList');
  RemClassRegistry.RegisterXSInfo(TypeInfo(ArrayOfENFuseShort), 'http://ksoe.org/EnergyproControllerService/type/', 'ArrayOfENFuseShort');

  InvRegistry.RegisterInterface(TypeInfo(ENFuseControllerSoapPort), 'http://ksoe.org/ENFuseController/message/', 'UTF-8');
  InvRegistry.RegisterDefaultSOAPAction(TypeInfo(ENFuseControllerSoapPort), 'http://ksoe.org/ENFuseController/action/ENFuseController.%operationName%');


end.
