//Контроллер Замен Электрических Шин
unit ENBusChangeController;

interface

uses InvokeRegistry, SOAPHTTPClient, Types, XSBuiltIns, EnergyProController,
  EnergyProController2, ENBusController, ENSubstation04Controller,
...
type
  ...
  ENBusChange            = class;                 { "http://ksoe.org/EnergyproControllerService/type/" }
  ...
  // ************************************************************************ //
  ENBusChangeRef = class(TRemotable)
  private
    Fcode: Integer;
  published
...
//???
  ...
  published
    ...
    property busRef : ENBusRef read FbusRef write FbusRef; 
...
{
  ENBusChangeFilter = class(TRemotable)
  private
...
//???
  ...
  ENBusChangeFilter = class(ENBusChange)
  private
    FconditionSQL: WideString;
    ForderBySQL: WideString;
  public
  ...
  ENBusChangeShort = class(TRemotable)
  private
    Fcode : Integer; 
    Fname : WideString;
    FinstallDate : TXSDate;	
    ...
    FworkerEquipChange : WideString;
    FbusRefCode : Integer; 
    FbusRefName : WideString;
    FbusRefInsulatornumberGen : TXSDecimal;
    FbusRefLength : TXSDecimal;
    FbusRefLocationScheme : WideString;
  ...
  published
    ...
    property busRefCode : Integer read FbusRefCode write FbusRefCode; 
    property busRefName : WideString read FbusRefName write FbusRefName; 
    property busRefInsulatornumberGen : TXSDecimal read FbusRefInsulatornumberGen write FbusRefInsulatornumberGen; 
    property busRefLength : TXSDecimal read FbusRefLength write FbusRefLength; 
    property busRefLocationScheme : WideString read FbusRefLocationScheme write FbusRefLocationScheme;
  ...
  ArrayOfENBusChangeShort = array of ENBusChangeShort;  // { "http://ksoe.org/EnergyproControllerService/type/" }

  ENBusChangeShortList = class(TRemotable)
  private
    FtotalCount: Integer;
    Flist: ArrayOfENBusChangeShort;
  public
  destructor Destroy; override;
  published
    property totalCount: Integer read FtotalCount write FtotalCount;
    property list: ArrayOfENBusChangeShort read Flist write Flist;
  end;

  ArrayOfInteger = array of Integer;

  // ************************************************************************ //
  // Namespace : http://ksoe.org/ENBusChangeController/message/
  // soapAction: http://ksoe.org/ENBusChangeController/action/ENBusChangeController.%operationName%
  // transport : http://schemas.xmlsoap.org/soap/http
  // style     : rpc
  // binding   : ENBusChangeControllerSoapBinding
  // service   : EnergyproControllerService
  // port      : ENBusChangeController
  // URL       : http://soap.ksoe.com.ua/energypro
  // ************************************************************************ //


  ENBusChangeControllerSoapPort = interface(IInvokable)
  ['{1a271a27-1a27-1a27-1a27-1a271a271a27}']
    function  add(const aENBusChange: ENBusChange): Integer; stdcall;
    procedure remove(const anObjectCode: Integer); stdcall;
    procedure save(const aENBusChange: ENBusChange); stdcall;
    function  getObject(const anObjectCode: Integer): ENBusChange; stdcall;
    function  getList: ENBusChangeShortList; stdcall;
    function  getFilteredList(const aENBusChangeFilter: ENBusChangeFilter): ENBusChangeShortList; stdcall;
    function  getScrollableList(const aFromPosition: Integer; const aQuantity: Integer): ENBusChangeShortList; stdcall;
    function  getScrollableFilteredList(const aENBusChangeFilter: ENBusChangeFilter; const aFromPosition: Integer; const aQuantity: Integer): ENBusChangeShortList; stdcall;
    function  getScrollableListByCondition(const aCondition: WideString; const aFromPosition: Integer; const aQuantity: Integer): ENBusChangeShortList; stdcall;
    function  getScrollableFilteredCodeArray(const aENBusChangeFilter: ENBusChangeFilter; const aFromPosition: Integer; const aQuantity: Integer): ArrayOfInteger; stdcall;

    procedure installBus(const anObject: ENBus; //Установить шину
      const aBusChObject: ENBusChange); stdcall;
    procedure uninstallBus(const anObject: ENBus; //Снять шину
      const aBusChObject: ENBusChange); stdcall;
    procedure changeBus(const anObjectOld: ENBus; //Заменить шину
      const aBusChObjectOld: ENBusChange;
      const anObjectNew: ENBus;
      const aBusChObjectNew: ENBusChange); stdcall;
  end;


implementation

  destructor ENBusChange.Destroy;
  begin
...
{  
  destructor ENBusChangeFilter.Destroy;
  begin
    if Assigned(FinstallDate) then
      installDate.Free;
    if Assigned(FuninstallDate) then
  ...
  destructor ENBusChangeShort.Destroy;
  begin
    if Assigned(FinstallDate) then
      installDate.Free;
    if Assigned(FuninstallDate) then
    ...
    if Assigned(FactDateGen) then
      actDateGen.Free;
    if Assigned(FbusRefInsulatornumberGen) then
      busRefInsulatornumberGen.Free;
    if Assigned(FbusRefLength) then
      busRefLength.Free;
  ...
  destructor ENBusChangeShortList.Destroy;
  var
    I: Integer;
  begin
...
initialization
  ...
  RemClassRegistry.RegisterXSClass(ENBusChange, 'http://ksoe.org/EnergyproControllerService/type/', 'ENBusChange');
  RemClassRegistry.RegisterXSClass(ENBusChangeRef, 'http://ksoe.org/EnergyproControllerService/type/', 'ENBusChangeRef');
  RemClassRegistry.RegisterXSClass(ENBusChangeFilter, 'http://ksoe.org/EnergyproControllerService/type/', 'ENBusChangeFilter');
  RemClassRegistry.RegisterXSClass(ENBusChangeShort, 'http://ksoe.org/EnergyproControllerService/type/', 'ENBusChangeShort');
  RemClassRegistry.RegisterXSClass(ENBusChangeShortList, 'http://ksoe.org/EnergyproControllerService/type/', 'ENBusChangeShortList');
  RemClassRegistry.RegisterXSInfo(TypeInfo(ArrayOfENBusChangeShort), 'http://ksoe.org/EnergyproControllerService/type/', 'ArrayOfENBusChangeShort');

  InvRegistry.RegisterInterface(TypeInfo(ENBusChangeControllerSoapPort), 'http://ksoe.org/ENBusChangeController/message/', 'UTF-8');
  InvRegistry.RegisterDefaultSOAPAction(TypeInfo(ENBusChangeControllerSoapPort), 'http://ksoe.org/ENBusChangeController/action/ENBusChangeController.%operationName%');
